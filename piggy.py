...
def ques_five():
    ...
    btn_22 = tk.Button(root, text="Для Данила", fg='red', width=50, height=3, font=30, command=final)
    btn_22.pack()
    lists.append(btn_22)
def finale():
    ...
    if hui == 5 and gay == 0 and exp == 0 and mate == 0:
        ...
        lbl_pikcha14.pack()
        lists.append(lbl_20)
        lists.append(lbl_result_15)
        lists.append(lbl_pikcha14)
        ...
        lbl_pikcha15.pack()
        lists.append(lbl_21)
        lists.append(lbl_result_16)
        lists.append(lbl_pikcha15)    
    
    
    else:
        labl = Label(root, text="Данил - клоун", font=("Comic Sans", 26))
        imgend = PhotoImage(file = 'finalend.png')
        lbl_pickhaend = Label(root)
        lbl_pickhaend.configure(image=imgend, width=imgend.width(),height=imgend.height())
        labl.pack()
        lbl_pickhaend.image = imgend
        lbl_pickhaend.pack()
        lists.append(labl)
        lists.append(lbl_pickhaend)
